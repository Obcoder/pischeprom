...
Для страницы и её API нужны вход в систему, подтверждённый email и право `sales_mailings.view` в guard `crm`. Заблокированные пользователи доступа не имеют. Роль `admin` в guard `crm` имеет доступ ко всем действиям раздела, как в других административных модулях.

Права `sales_mailings.edit`, `sales_mailings.send_test`, `sales_mailings.send_mass`, `sales_mailings.compliance_override`, `sales_mailings.manage_templates` и `sales_mailings.manage_suppression` разделяют редактирование, отправку и управление ограничениями. Меню и кнопки учитывают эти права; сервер проверяет их при каждом запросе. Менеджерам права раздела назначаются отдельно, массовая отправка автоматически не разрешается.

Миграция `2026_09_11_130000_register_sales_mailing_permissions` создаёт отсутствующие права и добавляет их существующей роли `admin` в guard `crm`, сохраняя другие назначения. На новых установках эти же права регистрирует `RolesAndPermissionsSeeder`. Production-деплой выполняется через GitHub Actions после push в `main`: `scripts/deploy-production.sh` автоматически запускает миграции и этот сидер, обновляет кэши и перезапускает SSR. Ручной запуск миграции при штатном деплое не требуется.

Причина ошибки `403` при открытии раздела: раньше контроллер требовал `sales_mailings.view`, но сидер не создавал ни одного права `sales_mailings.*`, а проверка Gate не учитывала роль `admin`. Ссылка при этом показывалась всем. Исправление сохраняет обязательную авторизацию и восстанавливает доступ администратора без необходимости предварительно перезапускать сидер.

Загрузка страницы и её справочников не обращается к Unisender. Отсутствующий ключ или отключённый провайдер показываются в настройках и не должны мешать открытию страницы. Заголовок страницы использует Inertia `Head`, поддерживаемый и браузерной, и серверной сборкой.
