...
`avito_messages` / `avito_auto_replies`, локальными ID изменённых чатов/сообщений
и признаками обновления списка и счётчиков. Тексты сообщений, данные клиентов и
настройки AI в WebSocket payload не передаются. Страница `/Ameise/avito` и канал
доступны активным пользователям с подтверждённой почтой: сотрудникам или
администраторам CRM. Гость перенаправляется на обычную форму входа; после входа
возвращается на страницу Avito, где подписка WebSocket запускается автоматически.
Проверка допуска к странице не зависит от `REALTIME_ENABLED`: сотрудник может
открыть архив и при сбое инфраструктуры. Обычные API Avito сохраняют существующую
модель доступа. Во встроенных чатах без авторизованной сессии или при выключенном
Realtime интерфейс показывает конкретную причину:
нужен вход, нет доступа, автообновление отключено на сервере либо не настроено.
Это не отдельный режим работы чатов. Для автоматических обновлений нужны
`REALTIME_ENABLED=true`, настроенные ключи Reverb, запущенные Reverb и worker
`database/realtime`, а также вход под активной учётной записью сотрудника или
администратора CRM с подтверждённой почтой. Локальный `.env`, созданный до
внедрения Realtime, не получает эти настройки автоматически при обновлении кода.

При временной ошибке WebSocket или недоступности сервера авторизации канала
клиент повторяет подключение автоматически с интервалом от 1 до 30 секунд.
Ожидание подписки ограничено 30 секундами. После успешной подписки клиент
догружает пропущенные изменения; в простое опроса API по таймеру нет.
Отказ в доступе (401/403/419) останавливает повторы до восстановления сессии
или явного повторного подключения.
...
Pinia объединяет близкие события, игнорирует повтор UUID и выполняет не чаще
одного фонового обновления в две секунды. Один `GET /api/avito/messenger/updates`
возвращает согласованный снимок счётчиков, текущую страницу с учётом фильтров
и только изменённые/новые сообщения выбранного чата. События другого чата
не загружают открытую переписку; изменения статуса синхронизации обновляют
только обзор. История не перечитывается целиком на каждое входящее сообщение.
HTTP 429 учитывает `Retry-After`; во время ожидания новые события объединяются.
Активный чат, черновик и позиция чтения сохраняются.
...
Сообщение и число непрочитанных сохраняются в одной транзакции до отправки
уведомления. При просмотре конца переписки в видимой активной вкладке клиент
подтверждает прочтение до последнего показанного сообщения. Новые сообщения,
пришедшие во время подтверждения, остаются непрочитанными. После подтверждения
один адресный запрос согласует состояние с сервером. Скрытая вкладка и просмотр
старой истории не подтверждают прочтение автоматически.
