...
После успешного развёртывания основной workflow также запускает
`scripts/check-production-avito.php`. В журнал Actions выводится JSON с
состоянием миграций Avito, обычной очереди, Reverb и подписки webhook,
полученной непосредственно от Avito. Тексты переписки, имена аккаунтов,
токены и полный URL webhook в отчёт не попадают. Проверка не отправляет
сообщений и не меняет режим AI или подписки. Режимы `off` и `shadow` могут
быть штатными; успешный деплой сам по себе не включает автоматическую отправку.
Если отчёт не удалось получить, workflow оставляет предупреждение; уже
развёрнутое приложение остаётся доступным.
