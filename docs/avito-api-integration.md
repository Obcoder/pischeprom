...
Актуальный [аудит автоответов AI и инструкция запуска](avito-auto-reply-audit.md) описывает исправления от 12 сентября 2026 года, разрешённые темы и диагностику причин отсутствия ответов.

[Устройство локального архива и условия долгосрочного восстановления](avito-messenger-archive.md)
описывают сохранение переписки независимо от Avito, границы первичного импорта
и необходимые резервные копии БД, вложений и ключей.
...
## Ограничения запросов в интерфейсе

Счётчик «Непрочитанных чатов» показывает количество чатов, а `unread_messages`
в API обзора — количество входящих непрочитанных сообщений. Системные сообщения
и удалённые записи не учитываются. Фильтр «Непрочитанные чаты» сохраняет поиск
и выбор аккаунта. При просмотре последних сообщений в активной вкладке чат
отмечается прочитанным; кнопка ручного подтверждения также доступна.

Повтор webhook без признака прочтения больше не сбрасывает уже сохранённое
прочтение. Счётчик пересчитывается вместе с сообщением до WebSocket-уведомления.
Миграция `2026_09_21_100000_repair_avito_unread_counts` исправляет старые значения
по сохранённым подтверждениям прочтения и пересчитывает итоги. Она не обращается
к Avito и не помечает неизвестные входящие прочитанными без подтверждения.

Локальные лимиты Ameise отделены от ограничений API Avito. Чтение данных и
изменяющие запросы используют независимые счётчики по 120 запросов в минуту
на пользователя (для гостя — на IP). Обновления списка чатов, истории и CRM
не расходуют лимит отправки сообщений.

У операций с дополнительными ограничениями отдельные счётчики: например,
30 текстовых сообщений, 20 отправок изображений и 10 запусков синхронизации
в минуту. Они также изолированы от запросов других разделов Ameise.
При достижении локального лимита сервер возвращает HTTP 429 с категорией
`local_rate_limit`, русским сообщением и временем ожидания в `retry_after`
и заголовке `Retry-After`.

Ранее числовые `throttle` без префикса использовали общий счётчик: фоновые
чтения могли заблокировать даже первую отправку, а вложенные ограничения
учитывали отправку дважды. Теперь общий лимит чтения и записи разделён,
а дополнительные лимиты имеют собственные префиксы. Внешние ограничения
Avito этим исправлением не меняются.
