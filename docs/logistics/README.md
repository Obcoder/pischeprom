...
Полная матрица запускается явным `--all` или кнопкой «Полная матрица». В неё входят все `logistics_cities` с `is_matrix_enabled=true`, проверенными координатами и routing-точкой; строятся все направленные пары. Лимит `LOGISTICS_MATRIX_MAX_CITIES_PER_REQUEST` применяется только к выбранному отображаемому фрагменту и не ограничивает полный фоновый расчёт. Ручные значения не перезаписываются.
