...
{
    protected $fillable = [
        'unit_id', 'unit_business_context_id', 'unit_product_match_id', 'good_id', 'unit_source_id',
        'prospecting_candidate_id', 'match_type', 'relevance', 'confidence', 'safe_rationale',
        'evidence_reference', 'evidence_hash', 'status', 'fit_status', 'compatibility_state', 'origin', 'rules_version',
    ...
    protected function casts(): array
    {
        return [
            ...
            'fit_status' => GoodOfferFitStatus::class,
            'compatibility_state' => ProductMappingState::class,
    ...
    protected static function booted(): void
    {
        static::saving(static function (self $match): void {
            ...
            $rawType = $match->getAttributes()['match_type'] ?? null;
            $type = $rawType instanceof UnitGoodMatchType
                ? $rawType
                : UnitGoodMatchType::from((string) $rawType);
            ...
            if (! $match->exists && (! $match->unit_product_match_id || ! ($match->getAttributes()['fit_status'] ?? null))) {
                throw new DomainException('New Good offer fits require a Unit Product match and fit status.');
            }
            if ($match->unit_product_match_id) {
                $productMatch = UnitProductMatch::query()->find($match->unit_product_match_id);
                if (! $productMatch
                    || (int) $productMatch->unit_id !== (int) $match->unit_id
                    || (int) $productMatch->unit_business_context_id !== (int) $match->unit_business_context_id) {
                    throw new DomainException('Good offer fit must belong to the selected Unit Product match.');
                }
                $mappedProductIds = Good::query()->without(['products'])->findOrFail($match->good_id)
                    ->products()->distinct()->pluck('products.id')->map(fn ($id) => (int) $id);
                if ($mappedProductIds->count() !== 1 || $mappedProductIds->first() !== (int) $productMatch->product_id) {
                    throw new DomainException('Good offer fit requires one exact Product mapping.');
                }
            }
        });
