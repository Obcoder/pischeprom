...
class AvitoAutoReplySafetyGuard
{
    ...
    private function matchesRestrictedBusinessTopic(string $text, string $compact): bool
    {
        $patterns = [
            ...
            '/(?:достав|привез|отправ|отгруз|курьер|заказ).{0,45}(?:когда|во\s+сколько|какого\s+числа|через\s+сколько)/u',
            '/(?:статус|состояни|что\s+(?:с|по)).{0,25}(?:заказ|доставк|курьер)|где\s+(?:сейчас\s+)?(?:(?:мо[йеим]\p{L}*|наш\p{L}*|эт\p{L}*)\s+)?(?:заказ|доставк|курьер)|(?:мо[йеим]\p{L}*|наш\p{L}*)\s+заказ.{0,30}(?:готов|собран|отправлен|доставлен|задерж)/u',
