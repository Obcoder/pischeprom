            ...
            <section v-else-if="activeTab === 'order'" class="crm-section">
                ...
                <template v-else>
                    ...
                    <div v-else class="order-lines">
                        <article v-for="item in orderItems" :key="item.good_id" class="order-line">
                            <div class="order-line__media">
                                <img v-if="item.image" :src="item.image" alt="" loading="lazy">
                                <v-icon v-else icon="mdi-image-off-outline" size="17" />
                            </div>
                            ...
                            <v-btn class="order-line__remove" icon="mdi-close" color="error" size="x-small" variant="text" title="Удалить товар из заказа" @click="removeOrderItem(item.good_id)" />
                        </article>
...
.crm-header { display: flex; min-height: 52px; flex: 0 0 52px; align-items: center; gap: 6px; padding: 7px 9px; border-bottom: 1px solid #30344d; background: #1b1e35; }.crm-header > div { min-width: 0; flex: 1; }.crm-header span, .crm-header strong { display: block; }.crm-header span { color: #9d88f4; font-size: 8px; font-weight: 800; letter-spacing: .12em; text-transform: uppercase; }.crm-header strong { margin-top: 2px; font-size: 11px; }
.crm-tabs { height: 38px; min-height: 38px; flex: 0 0 38px; border-bottom: 1px solid #2e324a; background: #191c31; }.crm-tabs :deep(.v-btn) { min-width: 0; min-height: 38px; padding: 0 5px; font-size: 8px; letter-spacing: 0; text-transform: none; }.crm-tabs :deep(.v-btn__content) { gap: 3px; }.crm-tabs b { display: grid; min-width: 15px; height: 15px; place-items: center; color: #fff; font-size: 7px; border-radius: 12px; background: #7558d7; }
.crm-content { min-height: 0; overflow-y: auto; flex: 1 1 auto; }.crm-section { display: grid; align-content: start; gap: 8px; padding: 9px; }
...
@media (max-width: 850px) { .avito-crm-pane { min-height: 0; height: 100%; border-top: 1px solid #30344d; border-left: 0; }.product-dialog__grid { grid-template-columns: 1fr; } }
</style>
