        ...
        <v-dialog v-model="createDialog" max-width="720">
            <v-card class="dialog-card"><v-card-title>Новый черновик из Good</v-card-title><v-card-text>
                ...
                <div class="good-results">
                    <label v-for="item in createGoods" :key="item.id" :class="{ active: createGoodId === item.id }"><v-radio v-model="createGoodId" :value="item.id" density="compact" /><div><strong>{{ item.name }}</strong><small>#{{ item.id }} · {{ item.is_published ? 'опубликован' : 'не опубликован' }} · {{ item.prices?.length || 0 }} цен · {{ item.media?.length || 0 }} фото</small></div></label>
                </div>
            </v-card-text><v-card-actions><v-spacer /><v-btn variant="text" @click="createDialog = false">Отмена</v-btn><v-btn color="deep-purple" :disabled="!createGoodId" :loading="createLoading" @click="createPublication">Создать черновик</v-btn></v-card-actions></v-card>
        </v-dialog>
...
.dialog-card { color: #edf0ff !important; background: #191c31 !important; }.dialog-card p { color: #a7adc7; font-size: 11px; }.good-results { overflow: auto; max-height: 420px; }.good-results label { display: flex; align-items: center; gap: 5px; padding: 5px; border-bottom: 1px solid #30344e; }.good-results label.active { background: rgba(109, 78, 218, .18); }.good-results label div { display: grid; }.good-results strong { font-size: 11px; }.good-results small { color: #8e94ae; font-size: 9px; }
