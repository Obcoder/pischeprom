...
async function calculate(refresh = false) {
    if (selectedIds.value.length < 2) { api.notify('Выберите минимум два проверенных города.', 'warning'); return }
    try {
...
async function calculateFullMatrix() {
    if (fullMatrixCityCount.value < 2) {
        api.notify('Для полной матрицы нужны минимум два проверенных города.', 'warning')
        return
    }
...
async function saveCitySettings() {
    try {
        ...
        } }, { success: 'Логистическая точка города сохранена.' })
...
<template>
    <section>
            ...
            <v-card-text>
                <div class="logistics-toolbar mb-0">
                    ...
                    <div class="logistics-toolbar__actions">
                        ...
                        <v-btn v-if="canManage" variant="tonal" color="green-darken-2" :disabled="selectedIds.length < 2" :loading="api.isPending('matrix-calculate')" @click="calculate(false)">Только отсутствующие</v-btn>
                        <v-btn v-if="canManage" variant="flat" color="green-darken-2" :disabled="selectedIds.length < 2" :loading="api.isPending('matrix-calculate')" @click="calculate(true)">Пересчитать</v-btn>
                    </div>
                </div>
            </v-card-text>
        </v-card>
        ...
        <v-card variant="outlined" class="matrix-card">
            ...
            <div v-else class="matrix-scroll">
                <table class="matrix-table">
                    ...
                    <tbody><tr v-for="from in selectedCities" :key="from.city_id"><th>{{ from.name }}</th><td v-for="to in selectedCities" :key="to.city_id" :class="`matrix-cell matrix-cell--${cell(from.city_id,to.city_id)?.status || 'missing'}`" :title="cellHint(cell(from.city_id,to.city_id))" @dblclick="canManage && from.city_id !== to.city_id && openManual(from.city_id,to.city_id)">
                        ...
                        <template v-else><span>—</span><small>не рассчитано</small><v-btn v-if="canManage" icon="mdi-pencil-outline" variant="text" size="x-small" @click="openManual(from.city_id,to.city_id)" /></template>
                    </td></tr></tbody>
                </table>
            </div>
            ...
            <v-card title="Рассчитать полную матрицу">
                <v-card-text>
                    <v-alert type="warning" variant="tonal" class="mb-4">
                        Будут обработаны все {{ fullMatrixCityCount.toLocaleString('ru-RU') }} включённых городов с проверенными routing-точками:
        ...
        <v-card v-if="canManage" variant="outlined" class="mt-4">
            ...
            <v-card-text>
                ...
                <v-list lines="two" max-height="320" class="overflow-y-auto border rounded">
                    <v-list-item v-for="city in cityCandidates" :key="city.city_id" :title="city.name" :subtitle="city.is_enabled ? `${city.is_verified ? 'координаты проверены' : 'нужна проверка'} · ${city.is_matrix_enabled ? 'в матрице' : 'отключён'}` : 'не включён в логистику'" @click="openCitySettings(city)">
        ...
        <v-dialog v-model="citySettingsDialog" max-width="680" persistent><v-card :title="`Routing-точка: ${citySettings.name}`"><v-card-text><v-row dense><v-col cols="6"><v-text-field v-model.number="citySettings.routing_latitude" type="number" step="0.0000001" label="Широта *" :error-messages="api.firstError('routing_latitude')" /></v-col><v-col cols="6"><v-text-field v-model.number="citySettings.routing_longitude" type="number" step="0.0000001" label="Долгота *" :error-messages="api.firstError('routing_longitude')" /></v-col><v-col cols="12" md="5"><v-select v-model="citySettings.coordinate_source" :items="[{title:'Существующие',value:'existing'},{title:'Ручные',value:'manual'},{title:'Импорт',value:'import'},{title:'OSM',value:'osm'}]" label="Источник" /></v-col><v-col cols="12" md="7"><v-text-field v-model="citySettings.source_reference" label="Ссылка / источник" /></v-col><v-col cols="6"><v-switch v-model="citySettings.is_matrix_enabled" label="Участвует в матрице" color="green" /></v-col><v-col cols="6"><v-checkbox v-model="citySettings.mark_verified" label="Проверить эту точку" color="green" /></v-col></v-row><v-alert type="info" variant="tonal" density="compact">Изменение координат пометит автоматически рассчитанные пары stale. Ручные пары сохранятся.</v-alert></v-card-text><v-card-actions class="justify-end"><v-btn @click="citySettingsDialog=false">Отмена</v-btn><v-btn color="green-darken-2" variant="flat" :loading="api.isPending('matrix-city-save')" @click="saveCitySettings">Сохранить</v-btn></v-card-actions></v-card></v-dialog>
    </section>
</template>
